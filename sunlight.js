...
export class SunLight {
	...
	// ---
	SolarOrientationCalculator() {
		this.a = "some val";
	}
}

SunLight.prototype = Object.assign(Object.create(THREE.Object3D.prototype), {
...
SunLight.prototype.SolarOrientationCalculator.prototype.getAzEl = function (
	lat_,
	lon_,
	date_ = new Date()
) {
...
SunLight.prototype.SolarOrientationCalculator.prototype._getJD = function (
	date_ = new Date()
) {
...
SunLight.prototype.SolarOrientationCalculator.prototype._calcAzEl = function (
	output,
	T,
	localtime,
	latitude,
	...
	var csz =
		Math.sin(this._degToRad(latitude)) * Math.sin(this._degToRad(theta)) +
		Math.cos(this._degToRad(latitude)) *
			Math.cos(this._degToRad(theta)) *
			Math.cos(haRad);
	if (csz > 1.0) {
		csz = 1.0;
	} else if (csz < -1.0) {
	...
	var azDenom =
		Math.cos(this._degToRad(latitude)) * Math.sin(this._degToRad(zenith));
	if (Math.abs(azDenom) > 0.001) {
		azRad =
			(Math.sin(this._degToRad(latitude)) * Math.cos(this._degToRad(zenith)) -
				Math.sin(this._degToRad(theta))) /
			azDenom;
...
SunLight.prototype.SolarOrientationCalculator.prototype._isLeapYear = function (
	yr
) {
	return (yr % 4 == 0 && yr % 100 != 0) || yr % 400 == 0;
};

SunLight.prototype.SolarOrientationCalculator.prototype._radToDeg = function (
	angleRad
) {
	return (180.0 * angleRad) / Math.PI;
};

SunLight.prototype.SolarOrientationCalculator.prototype._degToRad = function (
	angleDeg
) {
...
SunLight.prototype.SolarOrientationCalculator.prototype._calcSunRadVector =
	function (t) {
		...
		var e = this._calcEccentricityEarthOrbit(t);
		var R = (1.000001018 * (1 - e * e)) / (1 + e * Math.cos(this._degToRad(v)));
		return R; // in AU
	};
